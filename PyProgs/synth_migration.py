...
def generateSyntheticDirac(opdict,time_grids=None):
    ...
    ttimes={}
    data={}
    for index in range(len(opdict['syn_ix'])):
      ix=opdict['syn_ix'][index]
      iy=opdict['syn_iy'][index]
      iz=opdict['syn_iz'][index]
      # define origin time
      s_t0 = opdict['syn_otime'][index]

      it=int(round(s_t0/s_delta))

      s_amplitude=opdict['syn_amplitude'][index]
